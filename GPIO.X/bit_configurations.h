...
#define set_bit_Add(x, bit) (*((unsigned char*)x) = (*((unsigned char*)x)) | (1<<bit))
...
#define clr_bit_Add(x, bit) (*((unsigned char*)x) = (*((unsigned char*)x)) &~ (1<<bit))
...
#define toggle_bit_Add(x, bit) (*((unsigned char*)x) = (*((unsigned char *)x)^(1<<bit)))

//get bit functions
//get bit by sending the desired address and the bit number
#define get_bit(x, bit) ((x & (1<<bit)) > 0)
//get bit by sending the desired address and the bit number
#define get_bit_Add(x, bit) (((*((unsigned char*)x)) & (1<<bit)) > 0)
